...
const SearchBar: React.FC<{value: string; onChange: (s: string) => void}> = ({ value, onChange }) => {
  return (
    <div className="relative">
      <input value={value} onChange={e => onChange(e.target.value)} placeholder="Search roles, keywords…" className="w-full rounded-xl bg-white/[0.05] border border-white/10 p-3 text-sm placeholder:text-neutral-400 focus:outline-none focus:border-cyan-400/50"/>
    </div>
  );
}
...
const ExplorePage: React.FC<ExplorePageProps> = ({ jobs, setRoute }) => {
  ...
  const header = useMemo(() => (loading ? "Loading roles..." : `${filteredJobs.length} roles found`), [filteredJobs, loading]);
      ...
      <div className="grid md:grid-cols-[280px,1fr] gap-6">
        <aside className="space-y-4">
          <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4">
            <h3 className="text-sm font-semibold opacity-80">Search</h3>
            <div className="mt-2"><SearchBar value={query} onChange={setQuery}/></div>
            <p className="mt-3 text-xs text-neutral-400">Try “Data Scientist”, “labeling”, “compliance”…</p>
          </div>
        </aside>
        <section>
          <div className="mb-3 text-sm text-neutral-400">{header}</div>
          {loading && (
          ...
          {!loading && (
            filteredJobs.length > 0 ? (
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {filteredJobs.map(j => <JobCard key={j.id} job={j} open={id => setRoute({name: "job", id})}/>)}
              </div>
            ) : (
              <div className="rounded-xl border border-white/10 bg-white/[0.03] p-6 text-sm">
